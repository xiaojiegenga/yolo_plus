...
> 文档归属：本文件临时存放在 `main` 根目录，不属于任何单项实验分支。
>
> 生命周期：仅用于另一台电脑完成项目初始化与上下文接管；确认接管完成后可删除本文件并将删除提交推送到 `main`。
>
> 版本控制例外：项目普通学习笔记仍只在本地保存；本文件因承担一次性跨设备交接任务，按用户要求临时上传 GitHub。
...
```text
CLOUD_PROJECT_HANDOFF.md
```

需要继续 data-v1 Baseline 相关工作时，再创建并切换对应实验分支：
...
根目录 `CODEX_PROJECT_CONTEXT.md`、`code_plus.md` 和 `node.md` 仍是本地笔记，通常不会出现在另一台电脑的 GitHub clone 中；稳定的正式实验事实以 `experiment_records/` 为准。本文件完成一次性交接后应删除，不能代替长期实验记录。
