...
改变对应源码结构或损失因素，并使用新的 Run ID。`000` 已完成；当前配置为
`data-v2-abl-drb-scsa-p34-b16-s42.yaml`，全部 C3k2 使用 DRB，原第 4、6 层后独立加入
SCSA。训练参数不变，从官方 `yolo26m-seg.pt` 迁移权重，训练由用户手动启动。
...
| `model` | 传给 `YOLO(...)` |
| `pretrained` | 可选官方权重名称或文件路径，自定义结构构建后加载；CLI 优先 |
