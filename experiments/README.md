...
- `data-v2-abl-010-dice-b16-s42.yaml`：正式消融 B 配置；模型结构保持 Baseline，
  实例掩膜损失增加 `0.5 × Soft Dice`，smooth=1.0。
...
改变对应源码结构或损失因素，并使用新的 Run ID。`000` 已完成；当前 `010` 已完成本地
实现与聚焦测试，待用户在云服务器手动预检和训练。
