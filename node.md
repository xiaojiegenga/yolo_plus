...
|---|---|---|---|
| Precision(M) | 0.717 | **0.753** | ↑ +0.036 (+5.0%) |
| Recall(M) | 0.620 | **0.633** | ↑ +0.013 (+2.1%) |
| **mAP50(M)** ⭐ | **0.683** | **0.700** | ↑ +0.017 (+2.5%) |
| mAP50-95(M) | 0.329 | **0.343** | ↑ +0.014 (+4.3%) |
| 卷叶螟 AP50(M) | 0.604 | **0.607** | ↑ +0.003 (+0.5%) |
| 钻心虫 AP50(M) | 0.763 | **0.793** | ↑ +0.030 (+3.9%) |
| 卷叶螟 Recall | ~0.48 | **0.552** | ↑ +0.072 (+15%) |
| Box mAP50 | 0.667 | **0.694** | ↑ +0.027 (+4.0%) |

#### 分析解读

**✅ 正面效果**:
1. **整体指标全面提升** — mAP50 +1.7pp, mAP50-95 +1.4pp，说明 CBAM 注意力确实增强了特征表达
2. **钻心虫提升最明显** (+3pp AP50) — 钻心虫体型较大，CBAM 的空间注意力有助于定位
3. **卷叶螟 Recall 大幅提升** (+7.2pp, 漏检率 52%→45%) — 通道注意力帮助检测到了更多之前漏掉的虫子
4. **mAP50-95 相对提升最大** (+4.3%) — 说明 mask 边缘质量也有所改善

**⚠️ 仍需改进**:
1. **卷叶螟 AP50 几乎不变** (+0.003) — 虽然 Recall 上去了，但 Precision 下降抵消了 AP50 收益。说明 CBAM 帮网络看到了更多疑似虫子，但区分能力还不够
2. **漏检率仍有 45%** — 极小的卷叶螟个体仍是挑战，需要更强的改进（如 P2 高分辨率层）

**总结**: CBAM 作为第一步改进温和但有效，所有指标无一下降。对于中期答辩，这是一个清晰的"正向改进"案例。但核心问题（卷叶螟漏检）的真正突破需要 v2 P2 高分辨率层。
