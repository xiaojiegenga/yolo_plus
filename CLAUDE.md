...
当前目标是验证两个有效的结构改进；先独立实现 E：Neck DySample，再根据结果决定 D1+E。
...
python -m pytest tests/test_srcbam_model.py -q
python -m pytest tests/test_srcbam_model.py -q -k zero_init   # 只跑单个测试
```
...
5. `ultralytics-main/tests/test_srcbam_model.py` 补聚焦测试：形状与反向、相对
