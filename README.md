...
正式 Baseline 分支为 `cloud/data-v2-5090`；当前改进 B 分支为
`feature/data-v2-abl-b-dice`。B 已完成本地实现，等待 A2 训练结束后在云端预检和训练。
...
- `010 Instance Dice`：`experiments/data-v2-abl-010-dice-b16-s42.yaml`（待云端训练）
...
```bash
git clone --branch feature/data-v2-abl-b-dice https://github.com/xiaojiegenga/yolo_plus.git yolo_plus
cd yolo_plus
```
...
RTX 5090 和表 1 参数均已冻结。B 的正式 Run ID 为
`data-v2-abl-010-dice-b16-s42`，完整 dry-run、预检、正式训练和回传命令见
`实验步骤.md`。
