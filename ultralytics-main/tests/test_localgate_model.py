"""Focused checks for the data-v2 F2 locally gated strip-context model."""
...
from ultralytics.nn.modules import C3k2LocalGate, LocalGateStripContext, StripContext
...
MODEL_PAIRS = (
    ("yolo26-seg.yaml", "yolo26m-p3-localgate-seg.yaml", 160),
)


def test_local_gates_preserve_f_and_control_strips_independently():
    """Unit gates retain learned F predictions; each gate controls only its strip."""
    torch.manual_seed(42)
    source = StripContext(256).eval()
    torch.nn.init.normal_(source.project.weight, std=0.02)
    candidate = LocalGateStripContext(256).eval()
    missing, unexpected = candidate.load_state_dict(source.state_dict(), strict=False)
    assert set(missing) == {"gate.weight", "gate.bias"} and not unexpected
    assert sum(p.numel() for p in candidate.parameters()) - sum(p.numel() for p in source.parameters()) == 130
    image = torch.randn(2, 256, 12, 16)
    features = []
    handle = candidate.project.register_forward_pre_hook(lambda module, args: features.append(args[0].clone()))
    with torch.no_grad():
        torch.testing.assert_close(candidate(image), source(image), rtol=0, atol=0)
        candidate.gate.bias.copy_(torch.tensor([-1.0986123, 1.0986123]))
        candidate(image)
    handle.remove()
    local, horizontal, vertical = features[0].chunk(3, dim=1)
    gated_local, gated_horizontal, gated_vertical = features[1].chunk(3, dim=1)
    torch.testing.assert_close(gated_local, local, rtol=0, atol=0)
    torch.testing.assert_close(gated_horizontal, 0.5 * horizontal)
    torch.testing.assert_close(gated_vertical, 1.5 * vertical)
...
def test_strip_context_identity_and_two_step_learning():
    ...
    torch.manual_seed(42)
    module = LocalGateStripContext(32)
    ...
    for step in range(2):
        ...
        if step == 1:
            for branch in (module.reduce, module.local, module.horizontal, module.vertical, module.gate):
...
def test_strip_context_forward_split_preserves_learned_context():
    ...
    torch.manual_seed(42)
    module = C3k2LocalGate(32, 32, n=2).eval()
    torch.nn.init.normal_(module.strip_context.project.weight, std=0.02)
    torch.nn.init.normal_(module.strip_context.gate.weight, std=0.05)
...
def test_strip_models_transfer_source_weights_and_keep_prediction_geometry():
    """The candidate preserves source predictions and P3/P4/P5 detection."""
    torch.manual_seed(42)
    ...
    for source_yaml, candidate_yaml, proto_size in MODEL_PAIRS:
        ...
        assert [i for i, layer in enumerate(candidate.model) if isinstance(layer, C3k2LocalGate)] == [16]
...
def test_strip_models_segmentation_loss_with_targets_and_background():
    """The segmentation head must backpropagate through the new projection for real and empty targets."""
...
def test_strip_models_checkpoint_fusion_preserves_learned_predictions():
    ...
    for _, yaml_name, _ in MODEL_PAIRS:
        ...
        torch.nn.init.normal_(context.gate.weight, std=0.1)
        torch.nn.init.normal_(context.gate.bias, std=0.1)
...
def test_strip_recipes_match_baseline():
    ...
    for config_name in ("data-v2-abl-f2-localgate-b16-s42.yaml",):
