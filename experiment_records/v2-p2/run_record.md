...
## 2026-07-30：为什么当前 P2 收益有限

### 先限定结论

当前 Ultralytics Val 结果只给出了“整体”和“分类别”指标，没有按目标尺寸分别输出
`AP_small / AP_medium / AP_large`。因此，现阶段能确认的是：

> P2 没有提高卷叶螟这一类别的整体 AP，但不能仅凭整体 AP 断言“小目标子集完全没有提升”。

要直接回答小目标是否受益，后续还需要在相同权重上增加按尺寸分桶的统一验证。

### 验证集目标尺寸统计

根据分割标签外接框和原图尺寸，将图像按保持比例缩放到 `imgsz=640` 后统计：

| 类别 | 实例数 | 中位宽×高（px） | COCO-small（面积 < 32²） | 极小（面积 < 16²） | COCO-medium |
|---|---:|---:|---:|---:|---:|
| all | 330 | 22.9 × 82.0 | 25.2% | 1.8% | 71.2% |
| Rice leaffolder | 281 | 19.7 × 85.0 | 27.4% | 1.8% | 69.4% |
| Rice stemborers | 49 | 41.3 × 67.0 | 12.2% | 2.0% | 81.6% |

这说明卷叶螟的主要困难更接近“窄、细长、与叶片纹理相似”，而不是绝大多数目标都小到
P3 完全无法表达。P2 对约四分之一的 small 样本可能有帮助，但该子集不足以自动推动全类别 AP。

### 已验证的结构事实

1. V2 新增 P2/4 检测、分类和 mask-coefficient 分支，但 Mask Proto 仍沿用 Baseline 的
   P3/P4/P5 路径，因此它主要增加“发现目标”的空间分辨率，并没有直接提高 Mask 原型分辨率。
2. Baseline 的 P3/P4/P5 Head 与 Proto 权重成功迁移；新增 P2 Neck 和 P2 Head 必须随机初始化。
3. P2 输出网格比 P3 更密，候选位置更多。结果中卷叶螟 Mask Recall `+0.030`，
   但 Precision `-0.029`、AP50 `-0.015`，符合“找出更多候选，同时增加误检”的表现。
4. V2 使用 batch=4，历史 Baseline 使用 batch=8；当前差值较小，不能把全部差异直接归因于 P2。

### 最可能的原因（需要后续实验验证）

1. **真正极小目标占比有限**：V2 改动最擅长的样本不是数据集主体。
2. **浅层纹理语义不足**：P2 保留更多叶脉、反光和杂乱纹理，也更容易把背景细节误认为虫害。
3. **新增分支从随机权重开始**：768 张训练图像对新 P2 Neck/Head 的监督有限，而原有三尺度分支已经获得预训练。
4. **候选数量上升但没有专门的小目标约束**：当前损失和匹配策略仍是通用设置，没有明确要求 P2 只负责小目标。
5. **Proto 未改变**：P2 可以改善“有没有找到”，却不能单独解决细长卷叶螟的 Mask 边缘与形状质量。

### 诊断结论

当前结果不是代码失败，而是 P2 的能力边界：它确实带来了轻微 Recall 增益，但新增高分辨率候选中的
误检抵消了收益。下一步先完成 `Baseline-b4`，隔离 batch 影响；若同 batch 下仍是相同趋势，则应把
“浅层语义增强/注意力抑制背景”和“按尺寸验证”列为比继续堆叠更复杂 P2 Head 更优先的方向。
