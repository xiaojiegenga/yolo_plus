...
- `check_drb_scsa.py`：7 项结构检查，覆盖前向尺寸、官方权重迁移、分割反向、保存加载和融合；加 `--cuda` 检查 AMP 反向与 FP16 推理，不读取数据集。
...
当前训练配置为 `experiments/data-v2-abl-drb-scsa-p34-b16-s42.yaml`，须显式传入
`--config`。配置顶层 `pretrained` 用于官方预训练权重迁移；模型的
`baseline_layer_map` 负责插入 SCSA 后的层号对应，训练器重建同结构模型时不重复映射。
`实验步骤.md` 包含云端检查、训练、日志打包以及按当前主机和端口下载的 SCP 命令。
