...
本次云端结构检查通过后，用户决定跳过 10 epoch 数据训练预检，在 tmux 手动完成正式训练。
实际 batch=16 的显存、Val、权重保存和数值状态已通过完整 Run 核验。
...
## 9. 正式实验结果（2026-09-08 登记）

实际训练 commit `b8001b6`，完整 300 epoch，best epoch 282。Mask mAP50 / mAP50-95
为 0.70130 / 0.35116，较 000 下降 0.01002 / 0.01232；两类别日志 Recall/AP 均下降。
训练时间 82.22 分钟（+11.69%），峰值显存 19.3 GB（+3.7 GB），全部 CSV 数值与权重有限。

配对评估的 174 个小卷叶螟中，固定 IoU=0.50/conf=0.25 的正确检出 107→112、
误检 123→149；Recall 提高 2.874 个百分点，但小目标 AP50 / AP50-95 下降
4.444 / 1.964 个百分点。新增 P2 路径带来了召回与误检的取舍，未形成综合收益。
本次 C 未通过门控，不进入组合；结论限定于当前实现、冻结配方和 seed=42。

证据：[C 正式记录](../experiment_records/runs/data-v2-abl-001-p2head-b16-s42.md)、[小目标专项协议与结果](../experiment_records/evaluations/data-v2-c-small-val.md)。
