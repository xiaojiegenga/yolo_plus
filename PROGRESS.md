...
当前分支 `feature/data-v2-abl-drb-scsa` 的全 C3k2-DRB + P3/P4 SCSA 正式训练已完成
300 epoch，结果与日志已回传并登记为负结果。best epoch 231，Mask mAP50 / mAP50-95
为 0.67664 / 0.31719，较正式 000 下降 3.468 / 4.629 个百分点。
下一步讨论 DRB 封装与替换范围，不直接把当前结构列为有效改进与 D1 组合。
...
- CPU / CUDA 检查及 10 epoch 预检已完成，正式训练正常结束。
...
- 本轮已登记 `comparison.csv` 与总表表 18；正式分析见
  `experiment_records/runs/data-v2-abl-drb-scsa-p34-b16-s42.md`。
- 源码诊断：整个 Bottleneck 被纯深度卷积 DRB 替换，内部通道混合、激活和局部残差随之移除；
  外层 CSP 保留。优先考虑封装过度简化及预训练复用减少，因果贡献尚未分离。
- best.pt 两处 SCSA β 为 0.27368 / 0.32153，注意力并未一直保持关闭。
...
1. 保留本轮负结果；训练正常，当前整体结构未达到精度提升目标。
2. 优先讨论保留通道混合、激活与局部残差的 DRB 封装，再决定是否收窄替换范围。
3. 下一版由用户确定后再修改源码、配置和 Run ID；当前不启动训练。
4. Val 选方案，Test 只在最终方案与推理阈值冻结后统一执行。
...
- 本轮训练源码 commit 为 `3a19da2`；结果登记包含正式 Run 分析、总表表 18 和 `comparison.csv`。
- 本轮训练已完成，`实验步骤.md` 保留其复现流程；新训练需先确定新方案与 Run ID。
