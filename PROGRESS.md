...
- E 独立开发分支 `feature/data-v2-abl-e-dysample`，从正式 Baseline `c0f4f35` 创建；实现提交 `5d91a2b`。当前本地位于 A 总体分析分支。
...
- 本次同步电脑的 Git 根目录：`E:\study\graduate_sec\论文撰写\模型训练`；当前为总体分析分支 `feature/data-v2-abl-a-attention`；E 独立开发分支为 `feature/data-v2-abl-e-dysample`。
...
- 上一台分析电脑的 `.venv` 基于 `D:\tool\Anaconda3\envs\yolo26`：Python 3.10.19、PyTorch 2.10.0+cu130、Ultralytics 8.4.80；这些环境信息不代表本次同步电脑的安装状态，云端正式环境仍以表 3 和 Run 日志为准。
